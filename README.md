...
- 文件名（去掉 .txt 扩展名）会自动作为模板标题显示在对话框中
- 脚本会自动扫描 `templates/` 目录中的所有 .txt 文件
- 文件按文件名排序显示

**默认模板文件**：
- `1️⃣ TIENDA MOTO ELITE CATIA.txt` - 店铺信息模板
- `2️⃣ 问候模板.txt` - 问候模板
- `3️⃣ 感谢模板.txt` - 感谢模板
- `4️⃣ 确认模板.txt` - 确认模板
- `5️⃣ 结束对话模板.txt` - 结束对话模板
- `6️⃣ 自定义模板.txt` - 自定义模板
...
**注意事项**：
- 文件名（去掉 .txt）会显示为模板标题，建议使用有意义的文件名
- 支持 Emoji 表情符号在文件名和内容中
- 文件按文件名排序，可以使用数字或 Emoji 前缀来控制显示顺序
...
- **重大更新 QuickReplyTemplates.applescript**：
  - **所有模板现在都存储在 `templates/` 子目录中**
  - **模板标题自动从文件名读取**（去掉 .txt 扩展名），无需在脚本中硬编码
